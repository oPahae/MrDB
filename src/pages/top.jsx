...
function RankBadge({ rank, dark }) {
  ...
  if (rank === 3)
    return <span className={`${base} bg-gradient-to-br from-amber-500 to-amber-700 text-white`}>3</span>;
  return (
    <span
      className={`flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-xl font-black text-xs flex-shrink-0 ${
        dark ? "bg-white/4 border border-white/8 text-zinc-500" : "bg-gray-100 border border-gray-200 text-gray-400"
      }`}
    >
      {rank}
    </span>
...
function StatusBadge({ finished }) {
  return finished ? (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border bg-emerald-500/15 text-emerald-400 border-emerald-500/20">
      <CheckCircle2 size={9} />
      مكتمل
    </span>
  ) : (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border bg-sky-500/15 text-sky-400 border-sky-500/20">
      <Clock size={9} />
      مستمر
    </span>
  );
...
function SeriesRow({ sr, rank, dark }) {
  const router = useRouter();

  return (
    <div
      onClick={() => router.push(`/series?id=${sr.id}`)}
      className={`group flex items-center gap-2 sm:gap-4 border rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3 cursor-pointer transition-all duration-150 ${
        dark
    ...
    >
      ...
      {/* Thumbnail — portrait poster */}
      <div className="relative w-9 h-12 sm:w-12 sm:h-16 rounded-xl overflow-hidden flex-shrink-0 bg-zinc-900">
        {sr.img ? (
          <img
            src={sr.img}
            alt={sr.title}
      ...
      <div className="flex-1 min-w-0 space-y-0.5 sm:space-y-1">
        <h3
          className={`text-xs sm:text-sm font-bold leading-snug line-clamp-2 sm:truncate transition-colors group-hover:text-amber-400 ${
        ...
        {sr.otherTitle && (
          <p className={`text-[10px] sm:text-xs truncate ${dark ? "text-zinc-500" : "text-gray-400"}`}>
            {sr.otherTitle}
          </p>
        )}
        <div className="flex items-center gap-1 sm:gap-2 flex-wrap">
          <span className={`text-[10px] sm:text-xs ${dark ? "text-zinc-600" : "text-gray-400"}`}>
            {sr.seasonCount} موسم
          </span>
          <span className={`text-[10px] ${dark ? "text-zinc-700" : "text-gray-300"}`}>·</span>
          <span className={`text-[10px] sm:text-xs ${dark ? "text-zinc-600" : "text-gray-400"}`}>
            {sr.episodeCount} حلقة
          </span>
          {sr.start && (
            <>
              <span className={`text-[10px] hidden xs:inline ${dark ? "text-zinc-700" : "text-gray-300"}`}>·</span>
              <span className={`text-[10px] sm:text-xs hidden xs:inline ${dark ? "text-zinc-600" : "text-gray-400"}`}>
                {new Date(sr.start).getFullYear()}
              </span>
            </>
          )}
        </div>
        {/* Type + Status badges inline on mobile */}
        <div className="flex items-center gap-1.5 sm:hidden flex-wrap pt-0.5">
          <TypeBadge type={sr.type} />
          <StatusBadge finished={sr.finished} />
        </div>
      </div>

      {/* Type + Status — desktop only */}
        ...
        {sr.rating > 0 ? (
          <>
            <div className="flex items-center gap-0.5 sm:gap-1">
              <Star size={13} className="text-amber-400 fill-amber-400 sm:hidden" />
...
function Pagination({ page, pages, onPage, dark }) {
  ...
  return (
    <div className="flex items-center justify-center gap-1.5 sm:gap-2 mt-6 sm:mt-8">
      <button onClick={() => onPage(page - 1)} disabled={page === 1} className={`${base} ${inactive} ${page === 1 ? disabled : ""}`}>
      ...
      <button onClick={() => onPage(page + 1)} disabled={page === pages} className={`${base} ${inactive} ${page === pages ? disabled : ""}`}>
        <ChevronLeft size={13} />
      </button>
    </div>
  );
...
export default function TopSeries({ dark = true }) {
  ...
  const fetchSeries = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/top/series?${params}`);
  ...
  return (
    <>
      <Head>
        <title>أفضل السيريات — MrDB</title>
      ...
      <div dir="rtl" className={`pt-2 min-h-screen transition-colors duration-300 ${bg} ${text}`} style={{ fontFamily: "'Tajawal', sans-serif" }}>
        {dark && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h1
                  ...
                  style={{ fontFamily: "'Cairo', sans-serif" }}
                >
                  أقود السيريات فالتقييم
                </h1>
                <p className={`text-xs sm:text-sm mt-0.5 ${muted}`}>
                  {loading ? "جاري التحميل..." : `${total.toLocaleString()} سيري مرتبين من المقود تال الخاري`}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className={`${card} rounded-2xl sm:rounded-3xl p-3 sm:p-5 mb-4 sm:mb-6 space-y-3 sm:space-y-4`}>
            ...
            <form onSubmit={handleSearch} className="relative flex gap-2">
              <div className="relative flex-1">
                <Search size={13} className={`absolute top-1/2 -translate-y-1/2 right-3 pointer-events-none ${muted}`} />
                <input
                  type="text"
                  value={searchInput}
                  onChange={(e) => setSearchInput(e.target.value)}
                  placeholder="قلب على شي سيري..."
                  className={`w-full rounded-xl pr-9 pl-3 py-2.5 text-sm outline-none transition-all border ${
                    dark
              ...
              {searchInput && (
                <button
                  ...
                  className={`flex-shrink-0 w-10 flex items-center justify-center rounded-xl border transition-all ${
                    dark
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-0.5 scrollbar-hide">
              {TYPE_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl border text-[11px] sm:text-xs font-bold transition-all whitespace-nowrap flex-shrink-0 ${
                    type === opt.value
              ...
              {search && (
                <button
                  onClick={clearSearch}
                  className="flex-shrink-0 flex items-center gap-1 text-[11px] sm:text-xs font-bold text-rose-400 hover:text-rose-300 transition-colors px-2.5 sm:px-3 py-1.5 rounded-xl border border-rose-500/20 bg-rose-500/10 whitespace-nowrap"
                >
                  <X size={11} />
                  مسح البحث
                </button>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="hidden xs:flex items-center gap-2 sm:gap-4 px-3 sm:px-4 mb-2">
                ...
                <span className={`flex-1 text-[9px] sm:text-[10px] font-bold tracking-widest uppercase ${muted}`}>السيري</span>
                <span className={`hidden sm:block w-24 text-center text-[10px] font-bold tracking-widest uppercase ${muted}`}>النوع / الحالة</span>
              ...
              <div className="space-y-1.5 sm:space-y-2">
                {series.map((sr, i) => (
                  <SeriesRow key={sr.id} sr={sr} rank={rankStart + i} dark={dark} />
                ))}
              </div>

              <p className={`text-center text-[11px] sm:text-xs mt-3 sm:mt-4 ${muted}`}>
                {rankStart}–{Math.min(rankStart + series.length - 1, total)} من {total.toLocaleString()}
              </p>
