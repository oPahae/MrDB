...
export default function UpdateSeries({ dark = true }) {
  const router = useRouter();
  const { id } = router.query;

  const [form, setForm] = useState({
    title: "", img: "", trailer: "", descr: "",
    type: "anime", finished: true, start: "", end: "", maker: "",
  });
  ...
  // ── Load existing data from /api/admin/getSeries ──
  useEffect(() => {
    if (!id) return;
    const load = async () => {
      setLoadingData(true);
      try {
        const res = await fetch(`/api/admin/getSeries?id=${id}`);
        ...
        // seasons already contain { id, title, episodes: [{ id, number, title, img }] }
        setSeasons(data.seasons.length > 0 ? data.seasons : [emptySeason()]);
      } catch (err) {
  ...
  const addEpisode = (sIdx) =>
    setSeasons((s) => s.map((se, i) =>
      i === sIdx ? { ...se, episodes: [...se.episodes, emptyEpisode()] } : se
    ));
  const removeEpisode = (sIdx, eIdx) =>
    setSeasons((s) => s.map((se, i) =>
      i === sIdx ? { ...se, episodes: se.episodes.filter((_, j) => j !== eIdx) } : se
    ));
  const changeEpisode = (sIdx, eIdx, key, value) =>
    setSeasons((s) => s.map((se, i) =>
      i === sIdx
        ? { ...se, episodes: se.episodes.map((ep, j) => j === eIdx ? { ...ep, [key]: value } : ep) }
        : se
    ));
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/admin/updateSeries", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id, ...form, seasons }),
      });
      const data = await res.json();
  ...
  const addSeasonBtn = dark ? "bg-white/5 border-white/10 text-zinc-400" : "bg-white border-gray-200 text-gray-500 shadow-sm";
  ...
  if (notFound) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center gap-4 ${bg} ${text}`}>
        ...
        <p className="font-black text-lg" style={{ fontFamily: "'Cairo', sans-serif" }}>السلسلة غير موجودة</p>
        <NextLink href="/admin" className="text-sm text-amber-400 hover:underline">العودة للوحة التحكم</NextLink>
      </div>
    );
  }
  ...
  return (
    <>
      <Head>
        <title>تعديل: {form.title} — مرايا Admin</title>
          ...
          <div className="flex items-center justify-between mb-10">
            <div>
              ...
              <p className={`text-sm mt-1 ${muted}`}>
                {form.title ? `تعديل: ${form.title}` : "تعديل بيانات السلسلة"}
              </p>
            </div>
            <button
            ...
            <div className="space-y-4">
              <div className={`${card} rounded-3xl p-5 sticky top-6`}>
                ...
                <NextLink
                  href={`/series?id=${id}`}
                  target="_blank"
                  className={`mt-3 w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border text-xs font-bold transition-all ${
                    dark
